...
//doesn't work
    ...
    public void insertIntoClasses(String course, String section) {
        try {
            Class.forName(driver).newInstance();
            Connection conn = DriverManager.getConnection(url + dbName, userName, password);
            System.out.println("Success!");
            Statement st = conn.createStatement();
            int val = st.executeUpdate("INSERT INTO classes (idClasses, className, classSection) VALUES('" + course + "_" + section + "'),'" + course + "','" + section + "');");
            if (val == 1) {
                System.out.print("Successfully inserted value");
            }
            System.out.println(stmt);
     ...
     try {
            stmt = this.conn.prepareStatement(
                    "UPDATE classes "
                    + "SET BOOLEAN = true WHERE className = ? and classSection = ?");
